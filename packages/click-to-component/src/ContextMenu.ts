...
const ComponentContextMenu = (props: ContextMenuProps, ref: any) => {
  const { onClose } = props
  ...
  const mergedReferenceRef = React.useMemo(() => mergeRefs([ref, reference]), [ref, reference])
...
<${FloatingOverlay} key="click-to-component-overlay" lockScroll>
      ...
      style: {
        ...
        onClose?.((refs.floating.current as any)!.returnValue)
        setOpen(false)
      }}
              open
            >
...
export const ContextMenu = React.forwardRef(ComponentContextMenu)
